...
describe "Crawler.new" do
  ...
  context "#url_to_scrape" do
    it "returns the craigslist search url" do
      crawler.url_to_scrape.should eq 'http://sfbay.craigslist.org/search/hhh?srchType=A&addTwo=&addThree=&maxAsk=&minAsk='
      option2 = {:location => 'sfbay', :section => 'hhh', :cat => true, :min_price => 3000, :bedrooms => 3, :user_id => 1 }
      Crawler.new(option2).url_to_scrape.should eq 'http://sfbay.craigslist.org/search/hhh?srchType=A&bedrooms=3&addTwo=purrr&addThree=&maxAsk=&minAsk=3000'
      option3 = {:location => 'sfbay', :section => 'hhh', :sub_region => 'sby', :max_price => 2000, :bedrooms => 4, :query => 'view', :dog => true, :user_id => 1 }
      Crawler.new(option3).url_to_scrape.should eq 'http://sfbay.craigslist.org/search/hhh/sby?srchType=A&sub_region=sby&bedrooms=4&query=view&addTwo=&addThree=wooof&maxAsk=2000&minAsk='
    end
  end

  context "#urls_of_listings" do
    it "returns an array of all the listing urls" do
      crawler.urls_of_listings[0].should match /http:\/\/sfbay.craigslist.org/
    end
  end

  context "extracts the relavent information from each listing" do
    it "returns an array listing objects" do
      crawler.listings.first.should be_an_instance_of Listing
    end
  end

  context "save listings into the database" do
    it "saves all listings generated by the crawler to the database" do
      crawler.listings.first.id.should be_an_instance_of Fixnum
    end
  end
end